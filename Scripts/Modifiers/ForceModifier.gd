...
@export var AddForce : float = 0
@export var AddForcePercent : float = 0
...
var _lifetimeModifier : Modifier
...
func _enter_tree():
	...
	_lifetimeModifier = Modifier.create("Force", _gameObject)
	_lifetimeModifier.setName(Name)
	_lifetimeModifier.setAdditiveMod(AddForce)
	_lifetimeModifier.setMultiplierMod(AddForcePercent)
	_lifetimeModifier.allowCategories(ModifierCategories)
	_gameObject.triggerModifierUpdated("Force")
...
func on_level_up():
	...
	if _remainingLevelsToChange <= 0:
		_remainingLevelsToChange = ChangeEveryXLevels
		_lifetimeModifier.setMultiplierMod(_lifetimeModifier.getMultiplierMod() + ChangeMultiplier)
		_lifetimeModifier.setAdditiveMod(_lifetimeModifier.getAdditiveMod() + ChangeAdditive)
		_gameObject.triggerModifierUpdated("Force")

func _exit_tree():
	_lifetimeModifier = null
	_gameObject = null
