extends GameObjectComponent

@export var ModName : String = "Cooldown"
@export var Name : String

@export var AddBaseValue : float = 0
@export var AddMultiplier : float = 0
@export var RemoveAfterTime : float = -1
@export var ModifierCategories : Array[String]

@export var ChangeEveryXLevels : int = 0
@export var ChangeBaseValue : float = 0
@export var ChangeMultiplier : float = 0

func get_modifier_name() -> String:
	return Name

var _customMod : Modifier
var _remainingLevelsToChange : int = 0

func _enter_tree():
	initGameObjectComponent()
	if _gameObject == null:
		process_mode = PROCESS_MODE_DISABLED
		return
	process_mode = PROCESS_MODE_INHERIT if RemoveAfterTime > 0 or get_child_count() > 0 else PROCESS_MODE_DISABLED

	if ChangeEveryXLevels > 0:
		await Global.awaitWorldReady()
		Global.World.ExperienceThresholdReached.connect(on_level_up)
		_remainingLevelsToChange = ChangeEveryXLevels
	
	_customMod = Modifier.create(ModName, _gameObject)
	_customMod.setName(Name)
	_customMod.setAdditiveMod(AddBaseValue)
	_customMod.setMultiplierMod(AddMultiplier)
	_customMod.allowCategories(ModifierCategories)
	_gameObject.triggerModifierUpdated(ModName)


func on_level_up():
	_remainingLevelsToChange -= 1
	if _remainingLevelsToChange <= 0:
		_remainingLevelsToChange = ChangeEveryXLevels
		_customMod.setMultiplierMod(_customMod.getMultiplierMod() + ChangeMultiplier)
		_customMod.setAdditiveMod(_customMod.getAdditiveMod() + ChangeBaseValue)
		_gameObject.triggerModifierUpdated(ModName)


func _exit_tree():
	_customMod = null
	_gameObject = null


func _process(delta):
	if RemoveAfterTime < 0:
		return
	RemoveAfterTime -= delta
	if RemoveAfterTime <= 0:
		queue_free()
