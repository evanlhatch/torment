...
@export var AddRangePercent : float = 0.0
...
func _enter_tree():
	...
	_rangeModifier = Modifier.create("Range", _gameObject)
	_rangeModifier.setName(Name)
	_rangeModifier.setAdditiveMod(AddRange)
	_rangeModifier.setMultiplierMod(AddRangePercent)
	_rangeModifier.allowCategories(ModifierCategories)
	_gameObject.triggerModifierUpdated("Range")
...
func on_level_up():
	...
	if _remainingLevelsToChange <= 0:
		_remainingLevelsToChange = ChangeEveryXLevels
		_rangeModifier.setMultiplierMod(_rangeModifier.getMultiplierMod() + ChangeMultiplier)
		_rangeModifier.setAdditiveMod(_rangeModifier.getAdditiveMod() + ChangeAdditive)
		_gameObject.triggerModifierUpdated("Range")
