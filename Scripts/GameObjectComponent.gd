...
# NOTE: all the functionality of this class has to be copied over to the
# GameObjectComponent2D class! This is so that the base class can either
# be Node or Node2D, depending on the use...
...
func get_externalSource() -> GameObject:
	if _externalSourceGameObject != null and !_externalSourceGameObject.is_queued_for_deletion():
