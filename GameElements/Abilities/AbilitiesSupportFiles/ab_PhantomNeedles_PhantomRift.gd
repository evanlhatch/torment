...
func AquireAbility(modifiedTarget:Node) -> Array[Node]:
	...
	if baseNeedlesAbility == null:
		printerr("ab_PhantomNeedles_PhantomRift couldn't find its base ability on the player!")
		return []
	...
	printerr("ab_PhantomNeedles_PhantomRift could not find the bestowed needle emitter!")
	return []
