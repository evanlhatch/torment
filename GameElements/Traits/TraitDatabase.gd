...
var id_name := "database_id"
var entry_name := "filename"
...
func get_item(item_name : String):
	for item in __data:
		if item[entry_name] == item_name:
			return item
	return null


func get_item_value(item_name : String, value_name : String):
	for item in __data:
		if item[entry_name] == item_name:
...
static func load_database(database_script: String, path: String) -> TraitDatabase:
	var storage := load(database_script).new() as TraitDatabase
